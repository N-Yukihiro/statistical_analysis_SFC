...
`dplyr::glimpse`を利用して, データ構造を確認する
...
その際には`num_range()`を利用し, prefixに共通する列名, rangeに連番の範囲, widthに連番の桁数を指定する
...
行を選択するには`dplyr::filter()`を利用する
第一引数に利用するデータ, それ以降に行を選択する条件などを指定する
irisのデータからがく片の長さが平均(5.84)より大きい行を取り出す
...
重複を削除するには`dplyr`::`distinct()`を利用する
...
列を変換するにはdplyr::mutate()を利用する
第一引数に利用するデータ, それ以降に追加する列名とどのように変換するのかなどを指定する
irisのデータからがく片の長さを2倍したものと2乗したものをそれぞれtwiceとsquareに格納する
...
`pivot_longer()`の場合は, 第1引数に利用するデータ, 第2引数に残す列名,  第3引数に元の変数名を格納する列名, 第4引数に元の変数の値を格納する列名を指定する
ここでは先に各個体にidを振っている

irisのデータを使って, 種別以外の変数を縦型に変換する
...
`pivot_wider()`の場合は, 第1引数に利用するデータ, 第2引数に変数名が格納してある列名, 第3引数に変数の値が格納してある列名を指定する
...
irisのデータだと使いづらいので,  `band_*` のデータを使う
...
2つのデータセットの全ての行と列を結合するには`dplyr`::`full_join()`を使う
...
左側のデータセットにある行を基準に右側のデータセットを結合するには`dplyr`::`left_join()`を使う
...
## `inner_join()`
一致する行だけを残したい場合は`dplyr`::`inner_join()`を使う
...
一致する行だけを残し, なおかつ結合されるデータの列のみを残すには, `dplyr`::`semi_join()`を使う
...
一致しなかった行だけを選択するには`dplyr`::`anti_join()`を使う
...
データセットとデータセットを横に結合する(列を増やす)には`dplyr`::`bind_cols()`を使う
...
データセットとデータセットを縦に結合する(行を増やす)には`dplyr`::`bind_rows()`を使う

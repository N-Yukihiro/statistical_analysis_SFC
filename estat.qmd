...
検索する際には, `View()`関数を用いて視覚的に探しても良いし, `dplyr::filter()`関数などを利用しても良い.
...
統計データのメタ情報取得するには, `estat_getMetaInfo()`関数を利用し, `statsDataId`にIDを指定する.
...
データフレーム同士を列をキーにして結合するには, `dplyr::left_join()`関数を利用する.
